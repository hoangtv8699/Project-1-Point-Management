...
public class MH_GVDAO extends CommonDAO<MH_GV> {
    ...
    public MH_GV find(String maHP, String maLop, String hocky) {
        String sql = "select * from mh_gv where maHP=? and maLop=? and hocky=?";
        List<MH_GV> mhs = query(sql, new MH_GVMapper(), maHP, maLop, hocky);
        if (mhs.isEmpty()) {
            return null;
        }
        return mhs.get(0);
